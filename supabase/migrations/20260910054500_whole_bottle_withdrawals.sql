...
begin
  ...
  if exists (select 1 from public.withdrawals w join jsonb_to_recordset(p_rows) as r(id uuid, actual_qty numeric) on r.id=w.id where w.bottle_id is not null and r.actual_qty not in (0,1)) then
    raise exception 'Linked bottle quantity must be zero or one';
  end if;
