-- 00057_recurrence_attachments.sql
-- งานประจำแนบไฟล์/รูป/ลิงก์ได้ (เก็บเป็น jsonb บนแม่แบบ) → generate คัดไปทุก occurrence

ALTER TABLE task_recurrences ADD COLUMN IF NOT EXISTS attachments jsonb NOT NULL DEFAULT '[]'::jsonb;
  ...
  FOR rec IN SELECT * FROM task_recurrences WHERE active LOOP
    ...
    LOOP
      ...
      BEGIN
        ...
        -- คัดไฟล์แนบประกอบงานจากแม่แบบไปทุก occurrence
        IF jsonb_typeof(rec.attachments) = 'array' AND jsonb_array_length(rec.attachments) > 0 THEN
          INSERT INTO task_attachments (task_id, kind, url, name, phase, uploaded_by)
          SELECT v_task_id,
                 COALESCE(NULLIF(a->>'kind', ''), 'link')::task_attachment_kind,
                 a->>'url', NULLIF(a->>'name', ''), 'work', rec.created_by
          FROM jsonb_array_elements(rec.attachments) a
