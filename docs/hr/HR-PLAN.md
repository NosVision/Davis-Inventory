...
## สถานะ & งานถัดไป (บันทึก 2026-07-02 — ระหว่างรอคำตอบลูกค้า)

> ลูกค้ายืนยัน **เอาทุกฟีเจอร์ที่เสนอ** (12 ฟีเจอร์ + §A–J + ทุก batch) · scope นิ่งแล้ว เหลือเคลียร์คำถามชุดสุดท้ายก่อนเริ่ม build

### คำถามค้างลูกค้า (ส่งไปแล้ว 19 ข้อ)
> **triage:** ❗ต้องได้คำตอบจริงมีแค่ **ข้อ 1-2 (ชั่วโมงที่ 9 ของ 9+1) และข้อ 6 (วิธีหักลา)** — เดาผิด = จ่ายเงินผิด · ข้อ 5, 8-9 ออกแบบเป็น config รองรับทุกคำตอบแล้ว (chain เปิด/ปิดขั้น Manager ได้, pay_type ต่อคน, SC part-time = กรอกมืออยู่แล้ว §H) · ข้อ 16-17 = ข้อมูล seed ก่อน go-live ไม่บล็อก build · ที่เหลือ 12 ข้อ = default แก้ทีหลังได้ในระบบ
1. **ชั่วโมงที่ 9 ของคนทำ 9+1 คิดเงินยังไง** (รวมเงินเดือน หรือ OT? คนไม่มีสิทธิ์ OT ได้อะไร) — กระทบสูตร §A/§I
2. **สูตร OT ÷8 เท่ากันหมด หรือคนทำ 9 ชม. หาร 9**
3. กดพัก: GPS อย่างเดียว หรือ +selfie (ตอนนี้สมมติ GPS อย่างเดียว §I)
4. วันหยุดมาตรฐานกี่วัน/เดือน (ให้ตัวช่วยจัดตาราง §C ใช้เตือน)
5. **สลับวันหยุดใครอนุมัติ** — Manager→HR หรือ HR เดี่ยว (คำตอบ 2 รอบขัดกัน §C)
6. **"หักฟิค" ลาป่วย/ลากิจ** — เรต ÷30 แสดงบรรทัดแยก หรือ HR ตั้งเลขเอง (§H)
7. PH 13 หรือ 15 วัน เท่ากันทุกร้านไหม
8. **Part-time จ่ายรายชั่วโมง/รายวัน + เรต**
9. **Part-time เข้า SSO/OT/SC ไหม**
10. ใบเตือน "หักเดือนนี้/เดือนหน้า" ใครเลือก
11. SC ยังจ่ายวันที่ 15 เดือนถัดไปใช่ไหม
12. หัก % ใบเตือน = % ของ SC เดือนนั้นใช่ไหม
13. พนักงานเห็นคะแนนประเมินตอนไหน (ระหว่างงวด/หลังปิด)
14. ถ่วงน้ำหนักผู้ประเมินไหม (v1 = เฉลี่ยเท่ากัน)
15. 15 หัวข้อคู่มือ = แม่แบบเดือนแรกใช่ไหม
16. โครงสร้าง: กี่ร้าน / HR กี่คนดูร้านไหน / ผจก. ต่อร้าน
17. ร้านไหนสังกัดนิติบุคคลไหน (หัวสลิป)
18. ลิสต์ตำแหน่งงานเริ่มต้น (`hr_positions`)
19. ประกาศกด "ไม่รับทราบ" → เกิดอะไรต่อ

### งานที่ทำต่อได้เลย (ไม่ต้องรอคำตอบ)
- [ ] **อัปเดต checklist/PDF นำเสนอ** (`hr-checklist.html` + `HR-Feature-Checklist.pdf`) ให้ครอบคลุม scope ล่าสุด: §G ประเมิน, §H SC flow, §I เวลาทำงาน/พัก/ตำแหน่ง, §J (J4 ขอ OT, J5 ขอแก้เวลา, J6 ESS แก้ข้อมูล, J7 แผนก, J8 timesheet ที่ยังไม่มีใน checklist)
- [ ] **ปรับ `hr-eval-mockup.html`:** tier รองรับค่าติดลบ (หักจาก SC เช่น 50% → −200) + เพิ่มหน้าจำลอง "แบ่ง SC" (กองรวมกรอกมือ + หยอดต่อคน + หักอัตโนมัติจากใบเตือน/ประเมิน/มาสายซ้ำ ตาม §H)
- [ ] อัปเดต `hr-demo.html` ตามรายการ "ต้องอัปเดต/ต้องเพิ่ม" ด้านบน
- [ ] ร่าง **P0 migrations** (hr_employees + hr_positions + hr_departments + hr_companies + registry ซ่อน + i18n `hr` + พิกัดสาขา) — โครงไม่ขึ้นกับคำถามที่ค้าง

### งานที่ต้องรอคำตอบก่อน
- สูตร OT/ชั่วโมงที่ 9 (ข้อ 1-2) → ค่อย lock เอนจิน §A ส่วน OT
- Part-time (ข้อ 8-9) → เพิ่ม pay_type เข้า payroll engine
- สายอนุมัติสลับวันหยุด (ข้อ 5) → lock flow §C
- โครงสร้างร้าน/นิติบุคคล (ข้อ 16-17) → seed ข้อมูลจริง + scope `can_manage_hr`
