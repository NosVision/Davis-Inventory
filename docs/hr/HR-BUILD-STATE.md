...
> อัปเดตล่าสุด: 2026-07-02 (จบ Round 12 — **P2.1b check-in ESS เสร็จ** (/me/checkin GPS+geofence+selfie+watermark, aa78371) → ถัดไป P2.1c anti-VPN)

## สถานะรวม
- เฟสปัจจุบัน: **P2.1a ✅ + P2.1b ✅ (/me/checkin + /api/hr/ess/checkin) → ถัดไป P2.1c anti-VPN (ip-geo vs gps → is_vpn_suspect + รายงาน HR)**
...
## P2 — เวลา
- [~] P2.1 locations + เช็คอิน — **P2.1a + P2.1b เสร็จ** · P2.1a (0e665d2): 00086 hr_locations + hr_attendance + RLS · /api/hr/locations + /hr/locations · **P2.1b เสร็จ (aa78371)**: /api/hr/ess/checkin (POST auth-any: validate type/GPS/photo magic+3MB → resolve geofence server-side → upload selfie private bucket path → insert; GET today) + /me/checkin (type selector, GPS, camera→canvas watermark เวลาไทย+พิกัด, dev-upload bypass, Today list) + geo.ts (haversine) + `openBusinessDateBangkok()` (in-progress 6am-cutoff day) · **adversarial fraud review (Workflow 5-lens, 17 agents) → 5 confirmed แก้ครบ**: (1) geofence ตัดสินตาม radius ต่อร้าน (nearest ที่ "อยู่ใน" จริง ไม่ใช่ใกล้สุด) (2) in_geofence=null เมื่อไม่มี geofence (แยกจาก false) (3) single-store คง attribution, multi-store ambiguous→null (4) pre-upload guard: 429>20/min, 409 dup same-type ใน 30s (กันก่อน upload = ไม่มี orphan) · e2e chrome: in dist=0/true, out ~11.7km/false, no-geofence→null, dup→409(1 row/no orphan), business_date=Jul2 ✓ · typecheck เขียว · i18n th/en 26/26 · **เหลือ P2.1c anti-VPN** (ip-geo vs gps + datacenter range → is_vpn_suspect + รายงาน HR)
