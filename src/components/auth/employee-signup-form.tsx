...
import { Loader2, Check, X, Search, UserPlus, ShieldQuestion, Shield, Store } from 'lucide-react';
import { THAI_BANK_OPTIONS } from '@/lib/hr/bank-transfer';
import { ROLE_LABELS } from '@/types/roles';
import type { UserRole } from '@/types/roles';

/**
 * EmployeeSignupForm — the ONE public sign-up form (owner ask 2026-07-27: both link types must
 * look identical, differing only in whether the role is pre-bound). Used by:
 *   • /register/[token] — self-register (hr_registration_links): role lands as "ยังไม่ระบุ",
 *     HR verifies identity + assigns the role afterwards.
 *   • /invite/[token]  — invitation (staff_invitations): role + store come fixed on the link and
 *     show as badges; the account is usable right after sign-up.
 * All data calls go to /api/auth/hr-register, which resolves either token kind server-side.
 * Renders CONTENT ONLY (no page shell) so each page/layout provides its own card.
 */
...
export function EmployeeSignupForm({ token }: { token: string }) {
  ...
  const isInvite = ctx?.kind === 'invite';
  const inviteRoleLabel = ctx?.invite_role ? ROLE_LABELS[ctx.invite_role as UserRole] || ctx.invite_role : null;

  // Load link context (companies + positions + invite presets), or mark the link invalid.
  useEffect(() => {
    (async () => {
      try {
  ...
  // ── Render states (content only — the page/layout provides the card shell) ──
  if (loading) {
    return (
        ...
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
          {wasLinked ? (
            ...
            <>
              ...
              <br />สิทธิ์ระบบ <b className="text-gray-800 dark:text-gray-200">{inviteRoleLabel}</b>
              {ctx.store_name ? <> · สาขา <b className="text-gray-800 dark:text-gray-200">{ctx.store_name}</b></> : null}
            </>
          ) : (
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Invite links pre-bind the role + store — the ONLY difference from self-register. */}
      {isInvite && (
        <div className="flex flex-wrap items-center justify-center gap-2 rounded-lg bg-indigo-50 px-3 py-2 text-xs dark:bg-indigo-900/20">
          <span className="inline-flex items-center gap-1 font-semibold text-indigo-700 dark:text-indigo-300">
            <Shield className="h-3.5 w-3.5" />
            สิทธิ์ระบบ: {inviteRoleLabel}
          </span>
          {ctx.store_name && (
            <span className="inline-flex items-center gap-1 font-semibold text-indigo-700 dark:text-indigo-300">
          ...
          <span className="w-full text-center text-[11px] font-normal text-indigo-500 dark:text-indigo-400">
            กำหนดโดยลิงก์เชิญ — ใช้งานได้ทันทีหลังสมัคร
          </span>
        </div>
      )}
      ...
      {/* Company (only if the link isn't company-scoped). Position/role are assigned by HR
          after registration (self-register) or pre-bound by the invite link. */}
      {!ctx.company_id && (
        <div>
      ...
      {uStatus === 'taken' ? (
        ...
        <>
          ...
          <p className="text-center text-[11px] text-gray-400">
            {isInvite
              ? <>สมัครแล้วได้สิทธิ์ &quot;{inviteRoleLabel}&quot; ตามที่ลิงก์เชิญกำหนด — ใช้งานได้ทันที</>
              : <>สมัครแล้วจะได้สิทธิ์ &quot;ยังไม่ระบุ&quot; รอ HR กำหนดตำแหน่ง/สิทธิ์ให้อีกครั้ง</>}
          </p>
        </>
      )}
    </div>
