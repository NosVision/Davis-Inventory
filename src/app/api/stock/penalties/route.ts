...
export async function POST(req: NextRequest) {
  ...
  if (codeMeta.included_in_quota) {
    ...
    if (sopPoints >= threshold) {
      warning = await handleThresholdReached(service, {
        storeId,
        monthYear,
        sopPoints,
        issuedBy: user.id,
...
// When a store reaches the monthly SOP threshold: notify HQ/owner once, and in Auto mode issue a
// verbal warning to the store's head_bar (deduped per store/month via detail='stock_sop_monthly').
async function handleThresholdReached(
  service: ReturnType<typeof createServiceClient>,
  args: { storeId: string; monthYear: string; sopPoints: number; issuedBy: string; autoHr: boolean }
): Promise<{ issued: boolean; reason?: string }> {
  const { storeId, monthYear, sopPoints, issuedBy, autoHr } = args;
  ...
  if (!autoHr) return { issued: false, reason: 'manual_mode' };
  if (!headBar) return { issued: false, reason: 'no_head_bar' };

  // Auto mode: issue a verbal warning to the head_bar (money already comes from the fines).
