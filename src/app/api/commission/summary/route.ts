...
export async function GET(req: NextRequest) {
  ...
  let query = supabase
    ...
    .select('*, ae_profile:ae_profiles(id, name, nickname, email, bank_name, bank_account_no, bank_account_name, wht_cert_standing), staff_profile:profiles!commission_entries_staff_id_fkey(id, display_name, username, role), store:stores!commission_entries_store_id_fkey(id, store_name, store_code)')
  ...
  const aeMap = new Map<string, {
    ...
    /** อีเมลสำหรับส่งใบ 50 ทวิ — printed on the report and the PDF. */
    email: string | null;
