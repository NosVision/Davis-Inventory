...
// POST /api/hr/service-charge/allocations/[allocId]/deductions — add a MANUAL deduction line to
// a person's SC allocation (§H). Auto-computed lines come from recompute, not this route. A
// finalized pool is locked.
export async function POST(request: NextRequest, { params }: { params: Promise<{ allocId: string }> }) {
  const { allocId } = await params;
  const service = createServiceClient();

  // §P5.5: resolve the pool's store (allocation → pool) and gate before any write.
