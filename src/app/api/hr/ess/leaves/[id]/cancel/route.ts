...
// POST /api/hr/ess/leaves/[id]/cancel — the requester withdraws their OWN
// still-pending leave request (§E). Auth-any, but ownership is enforced: only the
...
export async function POST(
  ...
  const { data: updated, error } = await service
    ...
    .eq('status', 'pending')
    .select('id');
  if (error) return NextResponse.json({ error: 'Failed to cancel leave request' }, { status: 500 });
  if (!updated || updated.length === 0) {
    return NextResponse.json({ error: 'Already decided or not cancellable' }, { status: 409 });
  }

  // The cert file (if any) is intentionally retained as part of the audit trail.
