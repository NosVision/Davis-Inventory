...
// POST /api/hr/ess/profile-change-requests/[id]/cancel — the requester withdraws their
// OWN still-pending change request (§J6). Auth-any, but ownership is enforced: only the
...
export async function POST(
  ...
  const { data: updated, error } = await service
    ...
    .eq('status', 'pending')
    .select('id');
  if (error) return NextResponse.json({ error: 'Failed to cancel change request' }, { status: 500 });
  ...
  await logHrAudit(service, {
    ...
    reason: 'employee cancelled profile change request',
  });
