...
export async function POST(request: NextRequest) {
  ...
  const [userStoresRes, templatesRes] = await Promise.all([
    ...
    templateIds.length
      ? service.from('hr_shift_templates').select('id').eq('store_id', storeId).eq('active', true).in('id', templateIds)
  ...
  const badMember = userIds.find((u) => !userStores.get(u)?.has(storeId));
  ...
  if (badTemplate) return NextResponse.json({ error: 'A shift template is invalid for this store' }, { status: 400 });
  ...
  // Clears: delete this store's (user, date) rows. Error-checked so a failed removal is never
  // reported as saved (the client keeps the draft on a non-ok response and can retry).
  let clearFailed = 0;
