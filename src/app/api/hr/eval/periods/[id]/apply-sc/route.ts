...
import { scPoolMonthForEvent } from '@/lib/hr/pay-cycle';
...
// hr_sc_pools.period_month = scPoolMonthForEvent(period) — the pool paid on the 15th AFTER the period,
...
export async function POST(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: pools, error: poolErr } = await service
    .from(SC_POOLS)
    .select('id, status')
    .eq('period_month', scPoolMonthForEvent(period.period_month as string));
