...
// HR review queue for employee identity claims (owner flow 2026-07-05) — formerly the standalone
// /hr/identity-claims page, now the "คำขอยืนยันตัวตน" view inside the accounts tab of
// /hr/employees (owner ask 2026-07-27: one people surface). Who in the app says they are which
// payroll-sheet name: approve → hr_employees is created on the claimant's existing account;
// reject (with a note) returns the name to the pool. Also shows import coverage
...
export function IdentityClaimsManager({ onChanged }: { onChanged?: () => void }) {
  const isTh = useLocale() === 'th';
  const L = isTh
    ? { subtitle: 'ตรวจสอบว่าบัญชีผู้ใช้ไหนคือพนักงานคนไหน แล้วอนุมัติเพื่อผูกประวัติ', claimed: 'รอตรวจสอบ', unclaimed: 'ยังไม่ยืนยัน', linked: 'ยืนยันตัวตนแล้ว', queue: 'คิวรอตรวจสอบ', empty: 'ไม่มีคำขอค้าง', unclaimedList: 'รายชื่อที่ยังไม่มีใครยืนยัน', approve: 'อนุมัติ', reject: 'ปฏิเสธ', rejectReason: 'เหตุผลที่ปฏิเสธ (แจ้งพนักงาน)', approved: 'ยืนยันตัวตนแล้ว', rejected: 'ปฏิเสธแล้ว — ชื่อกลับเข้ารายการ', failed: 'ทำรายการไม่สำเร็จ', claimsAs: 'ระบุว่าเป็น', sheetPos: 'ตำแหน่งในชีท', started: 'เริ่มงาน', account: 'บัญชี', linkedList: 'รายชื่อที่ยืนยันตัวตนแล้ว', emptyUnclaimed: 'ไม่มีรายชื่อที่รอยืนยัน', emptyLinked: 'ยังไม่มีใครยืนยันตัวตน', noStore: 'ไม่มีสาขา', noStoreHint: 'รายชื่อนี้ไม่ได้ระบุสาขา — อนุมัติได้ แต่จะไม่ถูกจัดกะและไม่โผล่ในหน้าบันทึกเวลาของสาขาใด ต้องใช้ตัวเลือก "ไม่สังกัดสาขา" แทน' }
    : { subtitle: 'Verify which app account is which employee, then approve to link', claimed: 'Awaiting review', unclaimed: 'Unclaimed', linked: 'Linked', queue: 'Review queue', empty: 'No pending claims', unclaimedList: 'Names not yet claimed', approve: 'Approve', reject: 'Reject', rejectReason: 'Rejection reason (shown to the employee)', approved: 'Account linked', rejected: 'Rejected — name returned to the pool', failed: 'Action failed', claimsAs: 'claims to be', sheetPos: 'Sheet position', started: 'Started', account: 'Account', linkedList: 'Linked accounts', emptyUnclaimed: 'No unclaimed names', emptyLinked: 'No linked accounts yet', noStore: 'No venue', noStoreHint: 'This name has no venue — approving is fine, but they cannot be rostered and will not appear under any store on the timesheet. Use the "No venue" filter instead.' };
  ...
  const decide = useCallback(
    async (id: string, decision: 'approve' | 'reject') => {
      ...
      try {
        ...
        await load();
        // Approving links a profile ↔ employee — let the host tab refresh its badges/counters.
        onChanged?.();
      } catch (e) {
