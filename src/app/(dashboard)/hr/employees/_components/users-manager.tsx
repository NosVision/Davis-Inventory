...
/**
 * บัญชีผู้ใช้ & สิทธิ์ระบบ — formerly the standalone /users page, now the "accounts" tab of
 * /hr/employees (merged 2026-07-27 so HR has ONE surface for people). Vocabulary is deliberate:
...
export function UsersManager({ initialSearch }: { initialSearch?: string }) {
  ...
  const handleSaveRole = async () => {
    ...
    try {
      ...
      toast({ type: 'success', title: 'เปลี่ยนสิทธิ์ระบบเรียบร้อย' });
      setRoleEditUser(null);
      loadUsers();
    } catch (err) {
      toast({
        type: 'error',
        title: 'เปลี่ยนสิทธิ์ระบบไม่สำเร็จ',
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <select
          ...
          className="rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 dark:border-gray-600 dark:bg-gray-800 dark:text-white"
        >
          <option value="all">สิทธิ์ระบบทั้งหมด</option>
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-1.5">
                      ...
                      {linkedIds.has(u.id) ? (
                        ...
                        <Badge variant="default">ยังไม่ยืนยันตัวตน</Badge>
                      )}
                    </div>
                    <div
      ...
      >
        <div className="space-y-4">
          ...
          <Select
            ...
            options={[
              // Role labels shown in English always (single source: ROLE_LABELS).
      ...
      {/* Change System-Role Modal — สิทธิ์ระบบ ไม่ใช่ตำแหน่งงาน (ตำแหน่งงานแก้ที่แท็บพนักงาน) */}
      <Modal
        isOpen={!!roleEditUser}
      ...
      >
        <div className="space-y-3">
          ...
          <div className="rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-700 dark:bg-amber-900/20 dark:text-amber-400">
            สิทธิ์ระบบกำหนดว่าบัญชีนี้เปิดเมนูอะไรได้ — จะมีผลทันทีที่ผู้ใช้เข้าสู่ระบบครั้งถัดไป
            (ไม่เกี่ยวกับตำแหน่งงานในบริษัท ซึ่งแก้ได้ที่แท็บพนักงาน)
          </div>
        </div>
        <ModalFooter>
