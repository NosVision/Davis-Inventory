...
const STEPS: Step[] = [
  { t: 'เลือกบริษัท + เดือนจ่าย', b: 'ประวัติรอบเงินเดือนของบริษัทแสดงด้านซ้าย (ใหม่สุดบน) · แถบ PoolStrip เตือนความพร้อมของกอง SV/ทิปเดือนนั้น' },
  ...
  { t: 'ตรวจ/แก้ระหว่าง draft', b: 'แก้รายการประจำ (recurring) · ล.ย.01 · ใส่โบนัสรายคน (คำนวณ gross→ภาษี→net ใหม่อัตโนมัติ) · ใส่ภาษีเองเป็น fallback ได้', meta: <Meta tone="api">recurring · tax-allowance · bonus · tax-override</Meta> },
  ...
  { t: 'Finalize — ปิดยอด (ล็อก)', b: <><b>ต้องบัญชี "ตรวจครบ" ก่อน</b> — ถ้ายังไม่ confirm จะบล็อก (409) เว้นแต่ HR ใส่เหตุผล override (บันทึก audit) · ปิดยอดแล้ว <b>mark เบิก/claim เป็นจ่ายแล้ว</b></>, meta: <><Meta tone="api">POST finalize</Meta><Meta tone="gate">gate: accountant_confirmed</Meta></> },
...
const INPUTS: { n: 'N' | 'N−1'; title: string; items: React.ReactNode[] }[] = [
  ...
  { n: 'N', title: 'รายการประจำ (recurring)', items: [<><b>ค่าตอบแทน:</b> ครองชีพ · บ้าน · โทร · <b>เดินทาง</b> · ชดเชยวันหยุด</>, <><b>รายการหักประจำ</b></>] },
  { n: 'N−1', title: 'กองที่ยกมาเดือนก่อน', items: [<><b>Service Charge สุทธิ</b> = จัดสรร − หัก</>, <><b>ทิปสุทธิ</b></>, <><b>โบนัสประเมิน</b> (ลบ→หัก SV แยก)</>] },
  { n: 'N', title: 'อื่นๆ เข้าสลิป', items: [<><b>เบิก/claim</b> อนุมัติแล้วยังไม่จ่าย</>, <>โบนัส HR (เก็บนอกแถวสลิป)</>] },
...
const KEY_PTS: [string, React.ReactNode][] = [
  ...
  ['♻️', <><b>ภาษีที่บัญชีกรอก + โบนัส HR เก็บนอกแถวสลิป</b> → Recompute แล้วค่าเดิมยังอยู่</>],
