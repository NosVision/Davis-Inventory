...
interface RecurringGridProps {
  /** preselect this company (e.g. the payroll page's current company); falls back to the first */
  initialCompanyId?: string;
  /** fires after any successful mutation — the payroll page uses it to schedule a recompute */
  onChanged?: () => void;
}

// The company-wide recurring-items grid (rows = employees, columns = the typed item codes).
// Set once → every future payrun pulls it automatically; end month = auto-expiry the generate
// filter enforces. Full HR only (API-gated). Rendered by the /hr/payroll/recurring page AND
// inside a modal on /hr/payroll (owner ask 2026-07-15: modal buttons in one obvious spot).
export function RecurringGrid({ initialCompanyId, onChanged }: RecurringGridProps) {
  ...
  return (
    <div className="space-y-4">
      {/* toolbar: company + bulk add + search + filter + monthly commitment chips */}
      <div className="flex flex-wrap items-end gap-3">
        ...
        <label className="flex items-center gap-1.5 pb-2 text-sm text-gray-600 dark:text-gray-300">
          <input type="checkbox" checked={onlyWithItems} onChange={(e) => setOnlyWithItems(e.target.checked)} className="rounded border-gray-300" />
          {t('onlyWithItems')}
        </label>
        <div className="ml-auto flex flex-wrap gap-2 pb-1 text-xs">
