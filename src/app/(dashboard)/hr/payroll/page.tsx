...
export default function HrPayrollPage() {
  ...
  // Payroll slice to generate. '' = the ungrouped default run (what every run was before groups).
  const [payrollGroupId, setPayrollGroupId] = useState('');
  const [payrollGroups, setPayrollGroups] = useState<{ id: string; name: string; employee_count: number; confidential_count: number }[]>([]);
  ...
  // Payroll slices for this company. A slice holding confidential pay can only be generated by
  // someone with the grant — the API refuses otherwise — so the picker labels the count.
  const loadPayrollGroups = useCallback(async () => {
    if (!companyId) {
      setPayrollGroups([]);
      return;
    }
    try {
      ...
      setPayrollGroups((json.data?.groups ?? []) as typeof payrollGroups);
    } catch {
      setPayrollGroups([]);
    }
  }, [companyId]);
    ...
    try {
      const res = await fetch('/api/hr/payruns', {
        ...
        body: JSON.stringify({
          ...
          period_year: y,
          period_month: m,
          // Which slice to build. '' = the ungrouped default run, which is what every run was
          // before payroll groups existed.
  ...
  }, [companyId, month, payrollGroupId, t, loadPayruns, openPayrun]);
  ...
  const selectedPeriodRun = useMemo(() => {
    const [y, m] = month.split('-').map(Number);
    // Match the GROUP too: with payroll groups a company can have several runs in one period, and
    // picking the wrong one would mislabel the button and mis-apply the finalized lock.
    return (
      payruns.find(
        (p) =>
          p.period_year === y &&
          p.period_month === m &&
          (p.payroll_group_id ?? '') === payrollGroupId
      ) ?? null
    );
  }, [payruns, month, payrollGroupId]);
        ...
        <PageHeader
          ...
          actions={
            <>
              ...
              {/* Which slice of this company's payroll to build. Hidden until the company actually
                  has groups, so the common case stays a two-field form. */}
              {payrollGroups.length > 0 && (
                <label className="flex w-full flex-col text-xs font-medium text-gray-600 sm:w-auto dark:text-gray-400">
                  ...
                  <select
                    ...
                    className="control mt-1 w-full sm:w-auto"
                  >
                    <option value="">ยังไม่จัดกลุ่ม (งวดปกติ)</option>
                    {payrollGroups.map((g) => (
                      <option key={g.id} value={g.id}>
                        {g.name} ({g.employee_count} คน{g.confidential_count > 0 ? ' · ลับ' : ''})
                      </option>
                    ))}
                  </select>
                </label>
                  ...
                  <li key={p.id}>
                    ...
                    >
                      <span className="min-w-0 font-medium text-gray-900 dark:text-white">
                        ...
                        {/* Two runs can share a period once the company is split into groups —
                            without the name they are indistinguishable in this list. */}
                        {p.payroll_group?.name && (
                          <span className="ml-1.5 truncate text-xs font-normal text-indigo-500 dark:text-indigo-400">
                            · {p.payroll_group.name}
                          </span>
                        )}
                      </span>
