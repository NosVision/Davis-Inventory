...
export default function HrPolicySettingsPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4 p-4">
      ...
      {/* roster gate — whether an unrostered day may be clocked into at all */}
      <section className="space-y-3 rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
        <SectionHeading title={L.rosterGate} extra={<Gauge className="h-4 w-4 text-teal-500" />} />
        <div className="grid gap-2 sm:grid-cols-2">
          {([true, false] as const).map((on) => (
            <button
              key={String(on)}
              type="button"
              onClick={() => upd((d) => { d.attendance_requires_roster = on; })}
              aria-pressed={eff.attendance_requires_roster === on}
              className={`rounded-xl border p-3 text-left text-xs transition-colors ${eff.attendance_requires_roster === on ? 'border-indigo-400 bg-indigo-50 dark:border-indigo-500 dark:bg-indigo-900/20' : 'border-gray-200 hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-700/40'}`}
            >
              <span className="block font-semibold text-gray-800 dark:text-gray-100">
                {on ? L.rosterGateOn : L.rosterGateOff}
              </span>
              <span className="mt-0.5 block text-gray-500 dark:text-gray-400">
                {on ? L.rosterGateOnHint : L.rosterGateOffHint}
              </span>
            </button>
          ))}
        </div>
        <Button size="sm" isLoading={saving === 'attendance_requires_roster'} onClick={() => saveKey('attendance_requires_roster', { enabled: eff.attendance_requires_roster })}>{L.save}</Button>
      </section>
