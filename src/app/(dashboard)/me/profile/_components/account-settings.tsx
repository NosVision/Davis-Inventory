...
// ---------------------------------------------------------------------------
// Account settings — avatar, ชื่อเล่น, and notification preferences.
//
// Was the whole /profile page until 2026-08-07. The app had two profile screens (/profile from
// the avatar menu, /me/profile from the ESS hub) showing different halves of the same person,
...
export function AccountSettings() {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Written server-side: the only UPDATE policy on `profiles` is owner-only, so a browser
      // write matched zero rows for everyone else and PostgREST reported that as success.
  ...
  const handleSaveNickname = async () => {
    ...
    try {
      // Server-side for the same reason as the avatar above: a browser UPDATE on `profiles`
      // silently affected zero rows for every non-owner, so the new nickname survived until the
      // next refresh and then reverted. Trust the row the server echoes back, not the local input.
